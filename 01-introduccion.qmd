...
que cumple estas propiedades para tres elementos $A, B$ y $C$ de $\mathcal{X}$ [@xu2009clustering]:

1. *Positividad*: $0 \leq s(A,B) \leq 1,\; \forall A,B$.
2. *Simetría*: $s(A,B) = s(B,A)$.

Si además cumple las siguientes propiedades:

3. $s(A,B)S(B,C) \leq [S(A,B)+S(B,C)]S(A,C)$;
4. *Reflexividad*: $S(A,B)=1 \Leftrightarrow A=B$,
...
Una **función de distancia**, que podemos denotar como $DdA,B)$ mide también lo distintos que son 
...
$L_p$) del vector A. Así, para $p=1$ tenemos la *distancia Manhattan* ($L_1$), mientras que para $p=2$ tenemos
la popular *distancia Euclídea* ($L_2$).

Se denomina **matriz de distancias** a una matriz $D=(d_{E_{i}E_{j}});\, i,j=1,\dots,n$ que contiene todas las
distancias entre cada par de elementos $(E_i,E_j)$ de $\mathcal{X}$. Siempre que la función de distancia
utilizada cumpla la propiedad de *simetría*, esta matriz será *triangular*.
...
La @tbl-distance-funcs resume información básica sobre algunas funciones de distancia comúnmente empleadas.

| Función distancia               | Es métrica      |   Aplicaciones                                   |
|:--------------------------------|:---------------:|:-------------------------------------------------|
| Manhattan (*city block*)        | Sí              | Multiples: análisis de imagen, tiempos de viaje  |
| Euclídea                        | Sí              | Una de las funciones más utilizadas              |
| Minkowski                       | Sí, $p\geq1$    | Generalización de funciones de disancia básicas  |
| Mahalanobis                     | Sí              | Comparación entre distribuciones de valores      |
| Similaridad del coseno          | No(*)           | Agrupamiento y comparación de documentos         |
| Jaccard                         | Sí              | Datos binarios en ecología, genómica, etc.       |

: Algunas funciones de distancia muy frecuentes. {#tbl-distance-funcs}

(*): Se puede efectuar una transformación a una distancia "comparable" que sí cumple las propiedades de métrica.
